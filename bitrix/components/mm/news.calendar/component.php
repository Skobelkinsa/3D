...
if($this->StartResultCache(false, array($currentMonth, $currentYear, $todayYear, $todayMonth, $todayDay)))
{
	...
	$arSelectedFields = Array("ACTIVE", $arParams["DATE_FIELD"], "ID", "IBLOCK_ID", "SITE_ID", "DETAIL_PAGE_URL", "NAME", "LANG_DIR", "SORT", "IBLOCK_TYPE", "PREVIEW_TEXT", "PREVIEW_TEXT_TYPE", "DATE_ACTIVE_TO");
			...
			if(is_set($arDays[$tmpDate]))
			{
				...
				foreach($arDays[$tmpDate] as $dayNews)
				{
					...
					$arDay["events"][] = array(
						...
						"DATE_ACTIVE_TO"=>$dayNews["DATE_ACTIVE_TO"]
					);
